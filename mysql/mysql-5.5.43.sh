...
if [ `uname -m` == "x86_64" ];then
  rm -rf mysql-5.5.43-linux2.6-x86_64
  if [ ! -f mysql-5.5.43-linux2.6-x86_64.tar.gz ];then
	 wget http://zy-res.oss-cn-hangzhou.aliyuncs.com/mysql/mysql-5.5.43-linux2.6-x86_64.tar.gz
  fi
  tar -xzvf mysql-5.5.43-linux2.6-x86_64.tar.gz
  mv mysql-5.5.43-linux2.6-x86_64/* /alidata/mysql
else
  rm -rf mysql-5.5.43-linux2.6-i686
  if [ ! -f mysql-5.5.43-linux2.6-i686.tar.gz ];then
    wget http://zy-res.oss-cn-hangzhou.aliyuncs.com/mysql/mysql-5.5.43-linux2.6-i686.tar.gz
  fi
  tar -xzvf mysql-5.5.43-linux2.6-i686.tar.gz
  mv mysql-5.5.43-linux2.6-i686/* /alidata/mysql
fi
