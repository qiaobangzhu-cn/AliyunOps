#!/bin/bash
SRC_URI="http://zy-res.oss-cn-hangzhou.aliyuncs.com/activeMQ/apache-activemq-5.10.0-bin.tar.gz"
...
cd /alidata/install
if [ ! -s apache-activemq-5.10.0-bin.tar.gz ]; then
  wget -c $SRC_URI
fi
tar -xzvf apache-activemq-5.10.0-bin.tar.gz -C /alidata/
mv /alidata/apache-activemq-5.10.0/* /alidata/activemq
rm -rf /alidata/apache-activemq-5.10.0
