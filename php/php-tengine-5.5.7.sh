...
cat > /alidata/tengine/conf/fastcgi.conf << EOF
...
EOF
\mv /alidata/tengine/conf/vhosts/default.conf /alidata/tengine/conf/vhosts/default.conf.$DATE
cat > /alidata/tengine/conf/vhosts/default.conf << EOF
server {
    listen       80 default;
    server_name  _;
	...
	access_log  /alidata/tengine/logs/default.log;
}
EOF
rm -rf $PKG_NAME_DIR
tar zxvf $PKG_NAME
