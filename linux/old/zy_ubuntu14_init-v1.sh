...
#  apt-get install -y telnet iftop htop tmux source-highlight lrzsz atop numactl sendmail tcpdump sysstat chkconfig mailutils rpm
    ...
    apt-get update
    apt-get install -y lvm2 telnet iftop htop tmux source-highlight lrzsz atop numactl tcpdump sysstat nscd ntp
    apt-get install -y telnet iftop htop tmux source-highlight lrzsz atop numactl tcpdump sysstat chkconfig rpm
...
base_service_tunning(){
  ...
  if ! grep 'AllowGroups sshers' /etc/ssh/sshd_config >/dev/null;then echo "AllowGroups sshers www" >> /etc/ssh/sshd_config;fi
...
export LC_ALL=en_US.UTF-8
  ...
  ## bash prompt
  if ! grep 'source /etc/bash.bashrc' /etc/profile >/dev/null; then
...
'>> /etc/security/limits.conf
  ...
  ### add history date ###
  if ! grep 'export HISTTIMEFORMAT="%F %T' /etc/bash.bashrc >/dev/null; then
