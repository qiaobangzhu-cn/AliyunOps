...
LENGTH="8"
  ...
  while [ "${n:=1}" -le "$LENGTH" ]; do
      PASS1="$PASS1${MATRIX:$(($RANDOM%${#MATRIX})):1}"
      let n+=1
  done
...
while [[ "$HOSTNAME" != kd-* ]];do
echo "Wrong name,example:kd-xxx"; hostname_check
done
...
user_tunning(){
  ...
  /usr/sbin/useradd -u 500 -s /sbin/nologin www
  /usr/sbin/groupadd -g 4999 sshers
  useradd -G sshers zyadmin
  useradd -G sshers kdadmin

  echo "$PASS" | passwd zyadmin --stdin
  echo "$PASS1" | passwd kdadmin --stdin
  echo '
zyadmin        ALL=(ALL)       NOPASSWD: ALL
kdadmin        ALL=(ALL)       NOPASSWD: ALL
Defaults:zyadmin   !requiretty
Defaults:kdadmin   !requiretty
' >>/etc/sudoers
...
## Configure iptables
#wget http://git.jiagouyun.com/operation/operation/raw/master/linux/sys/script_firewall.sh -O /etc/iptables.sh
