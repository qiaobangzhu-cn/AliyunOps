...
# change hostname for server
  hostnamectl set-hostname $HOSTNAME 2> /dev/null || hostname $HOSTNAME 2> /dev/null
...
package_tunning(){
  ...
  yum install -y --nogpgcheck top iftop iotop atop htop netstat nc vmstat iostat telnet sar lsof tcpdump stare lrzsz rsync screen vim libnfnetlink libnetfilter_conntrack conntrack-tools tmux source-highlight numactl libnet sysstat mlocate 2> /dev/null
}
...
centos7_rclocal()
{
if lsb_release -a | grep -P "Release:.*7.*";then
...
if [ $? = 0 ]; then
    ...
    if lsb_release -a | grep -P "Release:.*7.*";then
    echo "start centos7_rc.local"
    centos7_rclocal
    echo "Finished centos7_rc.local"
    fi
