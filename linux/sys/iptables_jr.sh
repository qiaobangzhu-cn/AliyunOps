...
input_rules() {
  ...
  ###### SLB inbound -- only from the main interface - to be changed in the future if we want to access it only through the private IF
  slbip="10.158.0.0/16 10.159.0.0/16 10.49.0.0/16 100.64.0.0/10 100.109.0.0/16 100.97.0.0/16 100.116.0.0/15"
  for ip in $slbip
    do
      iptables -A INPUT -i eth0 -p tcp -m state --state NEW -m tcp -s $ip --dport 40022 -j ACCEPT
    done
