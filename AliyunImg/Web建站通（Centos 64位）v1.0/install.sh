...
function InstallReady()
{
	...
	cd $AMHDir/packages;
	wget http://zy-res.oss-cn-hangzhou.aliyuncs.com/amh/conf.zip;
...
function InstallLibiconv()
{
	...
	Downloadfile "${LibiconvVersion}.tar.gz" "http://zy-res.oss-cn-hangzhou.aliyuncs.com/amh/${LibiconvVersion}.tar.gz";
...
function InstallMysql()
{
	...
	Downloadfile "${MysqlVersion}.tar.gz" "http://zy-res.oss-cn-hangzhou.aliyuncs.com/amh/${MysqlVersion}.tar.gz";
...
function InstallPhp()
{
	...
	Downloadfile "${PhpVersion}.tar.gz" "http://oss.aliyuncs.com/aliyunecs/onekey/php/${PhpVersion}.tar.gz";
...
function InstallNginx()
{
	...
	Downloadfile "${NginxVersion}.tar.gz" "http://test-oracle.oss-cn-hangzhou.aliyuncs.com/${NginxVersion}.tar.gz";
	...
	if [ ! -d /usr/local/nginx ]; then
		...
		sed -i 's/'8888'/'$port'/g' /usr/local/nginx/conf/nginx.conf
...
function InstallPureFTPd()
{
	...
	Downloadfile "${PureFTPdVersion}.tar.gz" "http://zy-res.oss-cn-hangzhou.aliyuncs.com/amh/${PureFTPdVersion}.tar.gz";
...
function InstallAMH()
{
	...
	Downloadfile "${AMHVersion}.tar.gz" "http://zy-res.oss-cn-hangzhou.aliyuncs.com/amh/${AMHVersion}.tar.gz";
...
function InstallAMS()
{
	...
	Downloadfile "${AMSVersion}.tar.gz" "http://zy-res.oss-cn-hangzhou.aliyuncs.com/amh/${AMSVersion}.tar.gz";
...
function InitPass()
{
	echo "/bin/bash /alidata/init/initPasswd.sh" >> /etc/rc.d/rc.local
	echo '
...
sed -i "/LoginNum/,$ d" /root/.bashrc
	...
	mv /root/amh-install/initPasswd.sh /alidata/init/
	mv /root/amh-install/firstlogin.sh /alidata/init/
}
# AMH Installing ****************************************************************************
function lnfile()
{
	...
	mkdir -p /alidata/www
	ln -s /usr/local/nginx/ /alidata/server/nginx
	ln -s /usr/local/mysql/ /alidata/server/mysql
	ln -s /usr/local/php/ /alidata/server/php
	ln -s /home/wwwroot/ /alidata/www/
	ln -s /usr/local/php/var/log/  /alidata/logs/php
	ln -s /home/wwwroot/index/log/ /alidata/logs/nginx
...
##repair system BUG##
yum update -y bash
bash /root/amh-install/openssl-1.0.1h.sh
