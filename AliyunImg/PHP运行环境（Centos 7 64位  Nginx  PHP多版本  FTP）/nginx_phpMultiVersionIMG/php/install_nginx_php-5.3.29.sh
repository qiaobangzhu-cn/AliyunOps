#!/bin/bash
##/alidata/server/php-5.3.29
PHP_DIR=/alidata/server/$php53_dir
...
make install
cd ..
cp ./php-5.3.29/php.ini-production $PHP_DIR/etc/php.ini
#adjust php.ini
sed -i "s#; extension_dir = \"\.\/\"#extension_dir = "$PHP_DIR/lib/php/extensions/no-debug-non-zts-20090626/"#"  $PHP_DIR/etc/php.ini
...
##zend##
mkdir -p $PHP_DIR/lib/php/extensions/no-debug-non-zts-20090626/
if [ $machine == "x86_64" ];then
  ...
  mv ./ZendGuardLoader-php-5.3-linux-glibc23-x86_64/php-5.3.x/ZendGuardLoader.so $PHP_DIR/lib/php/extensions/no-debug-non-zts-20090626/
else
  ...
  mv ./ZendGuardLoader-php-5.3-linux-glibc23-i386/php-5.3.x/ZendGuardLoader.so $PHP_DIR/lib/php/extensions/no-debug-non-zts-20090626/
fi
 echo "zend_extension=$PHP_DIR/lib/php/extensions/no-debug-non-zts-20090626/ZendGuardLoader.so" >> $PHP_DIR/etc/php.ini
...
#self start
install -v -m755 ./php-5.3.29/sapi/fpm/init.d.php-fpm  /etc/init.d/php-fpm53
