...
switch_php() {
OPTION=$(whiptail --title "php默认使用5.4" --menu "您可以选择php进行切换\nhelp:需要用到键盘的tab键,方向键,回车键" 15 60 5  1 "php5.2.17" 2 "php5.3.29" 3 "php5.4.23" 4 "php5.5.7" 5 "php5.6.8" --fb --ok-button 确定 --cancel-button 取消 3>&1 1>&2 2>&3)
...
if [ "$OPTION" = "1" ]; then
     ...
     /etc/init.d/php-fpm stop
      ...
      rm -fr /alidata/server/php
      rm -fr /etc/init.d/php-fpm
      ln -s /alidata/server/php-5.6.8  /alidata/server/php
